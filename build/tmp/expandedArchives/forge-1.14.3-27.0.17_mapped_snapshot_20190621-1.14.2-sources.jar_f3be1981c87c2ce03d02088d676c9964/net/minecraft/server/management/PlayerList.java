...
public abstract class PlayerList {
   ...
   public void initializeConnectionToPlayer(NetworkManager netManager, ServerPlayerEntity playerIn) {
      ...
      if (serverworld == null) {
         playerIn.dimension = DimensionType.field_223227_a_;
      ...
      serverplaynethandler.sendPacket(new SJoinGamePacket(playerIn.getEntityId(), playerIn.interactionManager.getGameType(), worldinfo.isHardcore(), serverworld.dimension.getType(), this.getMaxPlayers(), worldinfo.getGenerator(), this.viewDistance, serverworld.getGameRules().func_223586_b(GameRules.field_223612_o)));
   ...
   public CompoundNBT readPlayerDataFromFile(ServerPlayerEntity playerIn) {
      CompoundNBT compoundnbt = this.server.getWorld(DimensionType.field_223227_a_).getWorldInfo().getPlayerNBTTagCompound();
   ...
   public ServerPlayerEntity createPlayerForUser(GameProfile profile) {
      ...
      if (this.server.isDemo()) {
         playerinteractionmanager = new DemoPlayerInteractionManager(this.server.getWorld(DimensionType.field_223227_a_));
      } else {
         playerinteractionmanager = new PlayerInteractionManager(this.server.getWorld(DimensionType.field_223227_a_));
      }

      return new ServerPlayerEntity(this.server, this.server.getWorld(DimensionType.field_223227_a_), profile, playerinteractionmanager);
   }
   ...
   public boolean canSendCommands(GameProfile profile) {
      return this.ops.hasEntry(profile) || this.server.func_213199_b(profile) && this.server.getWorld(DimensionType.field_223227_a_).getWorldInfo().areCommandsAllowed() || this.commandsAllowedForAll;
   }
   ...
   public void sendWorldInfo(ServerPlayerEntity playerIn, ServerWorld worldIn) {
      WorldBorder worldborder = this.server.getWorld(DimensionType.field_223227_a_).getWorldBorder();
      playerIn.connection.sendPacket(new SWorldBorderPacket(worldborder, SWorldBorderPacket.Action.INITIALIZE));
      playerIn.connection.sendPacket(new SUpdateTimePacket(worldIn.getGameTime(), worldIn.getDayTime(), worldIn.getGameRules().func_223586_b(GameRules.field_223607_j)));
   ...
   public ServerStatisticsManager getPlayerStats(PlayerEntity playerIn) {
      ...
      if (serverstatisticsmanager == null) {
         File file1 = new File(this.server.getWorld(DimensionType.field_223227_a_).getSaveHandler().getWorldDirectory(), "stats");
   ...
   public PlayerAdvancements getPlayerAdvancements(ServerPlayerEntity p_192054_1_) {
      ...
      if (playeradvancements == null) {
         File file1 = new File(this.server.getWorld(DimensionType.field_223227_a_).getSaveHandler().getWorldDirectory(), "advancements");
         File file2 = new File(file1, uuid + ".json");
