...
public class EnchantLiving extends LootFunction {
    ...
    public EnchantLiving(LootCondition[] conditionsIn) {
        super(conditionsIn);
    }
    ...
    public ItemStack apply(ItemStack stack, Random rand, LootContext context) {
        if (stack.getItem() == Items.BOOK) {
            ...
            ItemEnchantedBook.addEnchantment(stack, new EnchantmentData(EnchantmentLiving.LIVING_ENCHANTMENT, 1));
        } else {
        ...
        if (stack.getItem() instanceof ItemAxe)
            stack.setDisplayName(new TextComponentString(AXE_NAMES[rand.nextInt(AXE_NAMES.length)]));
        if (stack.getItem() instanceof ItemSword)
            stack.setDisplayName(new TextComponentString(SWORD_NAMES[rand.nextInt(SWORD_NAMES.length)]));
        if (stack.getItem() instanceof ItemPickaxe)
            stack.setDisplayName(new TextComponentString(PICKAXE_NAMES[rand.nextInt(PICKAXE_NAMES.length)]));
        if (stack.getItem() instanceof ItemSpade)
            stack.setDisplayName(new TextComponentString(SHOVEL_NAMES[rand.nextInt(SHOVEL_NAMES.length)]));
        if (stack.getItem() instanceof ItemBow)
            stack.setDisplayName(new TextComponentString(BOW_NAMES[rand.nextInt(BOW_NAMES.length)]));
        if (stack.getItem() instanceof ItemHoe && Config.COMMON.hoeNames.get())
            stack.setDisplayName(new TextComponentString(HOE_NAMES[rand.nextInt(HOE_NAMES.length)]));
    ...
    public static class Serializer extends LootFunction.Serializer<EnchantLiving> {
        ...
        public EnchantLiving deserialize(JsonObject object, JsonDeserializationContext deserializationContext, LootCondition[] conditionsIn) {
